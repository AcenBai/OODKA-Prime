#!/usr/bin/env python3
"""Evaluate native five-label LGE predictions in deployment class order."""
...
RAW_TO_FINAL = {3: 1, 5: 2, 4: 3, 1: 4, 2: 5}


def remap(array: np.ndarray) -> np.ndarray:
    output = np.zeros_like(array, dtype=np.int16)
    for source, target in RAW_TO_FINAL.items():
        output[array == source] = target
    return output
...
def main() -> None:
    ...
    for filename in sorted(
        name for name in os.listdir(args.pred_dir) if name.endswith(".nii.gz")
    ):
        ...
        rows.append({
            ...
            "dice": scores,
            "gt_present": [str(i) for i in CLASS_NAMES if (target == i).any()],
    ...
    summary = {
        ...
        "mean_of_case_macro_gt_present": float(np.mean([
            row["macro_gt_present"] for row in rows if row["macro_gt_present"] is not None
        ])),
        "mean_dice_per_class_gt_present": {
            str(class_id): float(np.mean([
